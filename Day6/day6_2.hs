...
main = do
  ...
  putStr $ show $ compute banks banks 1

compute :: [Int] -> [Int] -> Int -> Int
compute list memory initial
  | list == memory && initial == 0 = 0
  | otherwise = 1 + compute (distribute listToDistribute count start) memory 0
  where
    count = maximum list
